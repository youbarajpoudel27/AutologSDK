...
@available(iOS 13.0, *)
extension CameraViewControllerSDK {
    
    func setupGrainDetector() {
//        let modelURL = Bundle.main.url(forResource: "GrainModel", withExtension: "mlmodelc")
    
        do {
            let visionModel = try VNCoreMLModel(for: setupGrainDetectorMODEL()!)
    ...
    public func setupGrainDetectorMODEL() -> MLModel? {
           let bundle = Bundle.module // Swift 5.3 and later automatically provides this for packages
           guard let url = bundle.url(forResource: "GrainModel", withExtension: "mlmodelc") else {
               print("Failed to locate model in bundle.")
               return nil
           }
